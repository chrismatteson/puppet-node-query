...
declare -x PE_SERVER=$(puppet agent --configprint server)

declare -x NC_CURL_OPT="-s --cacert $PE_CA --cert $PE_CERT --key $PE_KEY --insecure"

show_node()
{
  curl $NC_CURL_OPT --insecure https://$2:8140/puppet/v3/node/$1?environment=production | python -m json.tool
}

if [ "$1" == "" ]; then
  echo "This tool requires an fqdn as input"
else
  show_node $1 $PE_SERVER
fi
